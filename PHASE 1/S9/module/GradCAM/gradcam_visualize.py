...
Original file is located at
    https://colab.research.google.com/drive/1tELYlZdwED6iVHaoMbC0N5ECya1mH7dv
"""
...
def visualize_grad_cam(mask, img, alpha=1.0):
...
def un_normalized(image, mean, std, out_type='array'):
...
class GradCAMView:
    ...
    def _cam_image(self, norm_image):
        ...
        for layer, gc in self.gradcam.items():
            ...
            cam_heatmap, cam_result = visualize_grad_cam(
                mask,
                un_normalized(norm_image, self.mean, self.std, out_type='tensor').clone().unsqueeze_(0).to(self.device)
            )
            heatmap[layer], result[layer] = convert_numpy(cam_heatmap), convert_numpy(cam_result)
        return {
            'image': un_normalized(norm_image, self.mean, self.std),
            'heatmap': heatmap,
            'result': result
        }
